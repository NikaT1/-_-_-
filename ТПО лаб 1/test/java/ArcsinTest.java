...
public class ArcsinTest {
    ...
    @ParameterizedTest(name = "{index} - arcsin({0})")
    @ValueSource(doubles = {-1, -0.93, -0.84, -0.65, -0.5, -0.34, 0, 0.28, 0.5, 0.67, 0.84, 0.9, 1})
    public void testArcsinCalcFunctionNormalValuesWithParamDelta(double number) {
        assertEquals(Math.asin(number), calc_arcsin(number, 85), 0.095 * Math.abs(number));
    }
    ...
    @ParameterizedTest(name = "{index} - sin(arcsin({0})) = {0}")
    @ValueSource(doubles = {-1, -0.93, -0.84, -0.65, -0.5, -0.34, 0, 0.28, 0.5, 0.67, 0.84, 0.9, 1})
    public void testArcsinCalcFunctionNormalValuesWithSin(double number) {
        assertEquals(number, Math.sin(calc_arcsin(number, 85)), 0.1);
    }
